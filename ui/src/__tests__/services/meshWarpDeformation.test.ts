/**
 * Mesh Warp Deformation Service Tests
 *
...
import type { WarpPin, WarpMesh, WarpWeightOptions } from '@/types/meshWarp';
import { createDefaultWarpPin, DEFAULT_WARP_WEIGHT_OPTIONS } from '@/types/meshWarp';
...
/**
 * Create a warp pin at a specific position
 */
function createWarpPin(
  x: number,
  y: number,
  options: Partial<WarpPin> = {}
): WarpPin {
...
describe('calculateWeights', () => {
  ...
  it('should calculate weights for multiple pins', () => {
    const vertices = new Float32Array([0, 0, 100, 0, 100, 100, 0, 100]);
    const pin1 = createWarpPin(0, 0);
    const pin2 = createWarpPin(100, 100);
  ...
  it('should apply radius-based falloff', () => {
    ...
    const pin = createWarpPin(0, 0, { radius: 50 });
  ...
  it('should apply stiffness reduction', () => {
    ...
    const normalPin = createWarpPin(0, 0, { stiffness: 0, radius: 100 });
    const stiffPin = createWarpPin(0, 0, { stiffness: 0.9, radius: 100 });

    // Disable normalization to see raw weight differences
    const opts: WarpWeightOptions = { ...DEFAULT_WARP_WEIGHT_OPTIONS, normalize: false };
...
describe('MeshWarpDeformationService', () => {
  ...
  describe('buildMesh', () => {
    it('should build a mesh from control points and pins', () => {
      ...
      const pins = [createWarpPin(50, 50)];
    ...
    it('should store pin rest states', () => {
      const controlPoints = createSquareControlPoints();
      const pins = [createWarpPin(25, 75)];
  ...
  describe('addPin', () => {
    ...
    it('should update weights after adding pin', () => {
      ...
      service.addPin(testLayerId, createWarpPin(50, 50));
    ...
    it('should do nothing for non-existent layer', () => {
      expect(() => {
        service.addPin('non-existent', createWarpPin(0, 0));
      }).not.toThrow();
    });
  });
  ...
  describe('removePin', () => {
    ...
    it('should do nothing for non-existent pin', () => {
      const controlPoints = createSquareControlPoints();
      service.buildMesh(testLayerId, controlPoints, [createWarpPin(50, 50)]);
  ...
  describe('getPins', () => {
    it('should return pins for layer', () => {
      const controlPoints = createSquareControlPoints();
      const pin1 = createWarpPin(25, 25);
      const pin2 = createWarpPin(75, 75);
  ...
  describe('updateMeshPins', () => {
    ...
    it('should recalculate weights', () => {
      ...
      const pin1 = createWarpPin(0, 0, { radius: 200, id: 'pin1' });
      const pin2 = createWarpPin(100, 100, { radius: 200, id: 'pin2' });
      service.buildMesh(testLayerId, controlPoints, [pin1, pin2]);
      ...
      service.updateMeshPins(testLayerId, [
        createWarpPin(0, 0, { radius: 200, id: 'pin1' }),
        createWarpPin(500, 500, { radius: 50, id: 'pin2' }), // Far away with small radius
      ]);
...
describe('meshWarpDeformation singleton', () => {
  afterEach(() => {
    meshWarpDeformation.clearAllMeshes();
  });

  it('should be a MeshWarpDeformationService instance', () => {
    expect(meshWarpDeformation).toBeInstanceOf(MeshWarpDeformationService);
  });

  it('should maintain state across uses', () => {
    const controlPoints = createSquareControlPoints();
    meshWarpDeformation.buildMesh('singleton-test', controlPoints, []);

    const mesh = meshWarpDeformation.getMesh('singleton-test');
...
describe('Deformation behavior', () => {
  ...
  it('should move vertices towards translated pin', () => {
    ...
    const pin = createWarpPin(50, 0, { radius: 100 });
  ...
  it('should not affect vertices outside pin radius', () => {
    ...
    const pin = createWarpPin(0, 0, { radius: 50 }); // Small radius
    service.buildMesh('test', controlPoints, [pin]);
  ...
  it('should handle multiple pins with overlapping influence', () => {
    ...
    const pin1 = createWarpPin(0, 50, { radius: 100 });
    const pin2 = createWarpPin(100, 50, { radius: 100 });
...
describe('Edge cases', () => {
  ...
  it('should handle many control points', () => {
    ...
    const pin = createWarpPin(50, 50, { radius: 200 });
  ...
  it('should handle many pins', () => {
    ...
    const pins = Array.from({ length: 20 }, (_, i) =>
      createWarpPin(Math.random() * 100, Math.random() * 100)
    );
  ...
  it('should handle zero radius pin', () => {
    const controlPoints = createSquareControlPoints();
    const pin = createWarpPin(50, 50, { radius: 0 });
...
// ============================================================================
// CREATE DEFAULT WARP PIN TESTS
// ============================================================================

describe('createDefaultWarpPin', () => {
  it('should create pin with default values', () => {
    const pin = createDefaultWarpPin('test-id', 100, 200);
  ...
  it('should create starch pin with stiffness 1', () => {
    const pin = createDefaultWarpPin('test-id', 0, 0, 'starch');
  ...
  it('should create rotation pin', () => {
    const pin = createDefaultWarpPin('test-id', 0, 0, 'rotation');
